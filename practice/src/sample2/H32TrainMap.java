...
public class H32TrainMap {//public class Solution {
	/**
	 * class Station
	 * <p>
	...
	private static class TrainMap {
		...
		public List<Station> shortestPath(String from, String to) {
			...
			queue.add(stations.get(from));
			while(!queue.isEmpty()) {
				Station currStation = queue.poll();
				System.out.println(currStation.getNeighbours().size());
                if(currStation.getName().equals(to))
                	break;
				for(Station neighbour:currStation.getNeighbours()) {
					if(!visited.containsKey(neighbour)) {
						 queue.add(neighbour);
						 visited.put(neighbour,currStation);
					}
				}
			}
